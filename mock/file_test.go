...
func TestFile_Write_EdgeCases(t *testing.T) {
	t.Run("test write with truncation at position 0", func(t *testing.T) {
		file := NewFile([]byte("Hello, World!"), "test.txt")

		// Write shorter data at position 0
		data := []byte("Hi")
		n, err := file.Write(data)
		assert.NoError(t, err)
		assert.Equal(t, 2, n)
		assert.Equal(t, []byte("Hi"), file.Bytes())
		assert.Equal(t, int64(2), file.pos)
	})

	t.Run("test write with truncation at position 0 with longer original", func(t *testing.T) {
		file := NewFile([]byte("Hello, World!"), "test.txt")

		// Write data that's shorter than original but longer than what we write
		file.Seek(0, io.SeekStart)
		data := []byte("Hi")
		n, err := file.Write(data)
		assert.NoError(t, err)
	...
	t.Run("test write without truncation", func(t *testing.T) {
		file := NewFile([]byte("Hello"), "test.txt")

		// Write at position 0 with data longer than original
		data := []byte("Hello, World!")
		n, err := file.Write(data)
		assert.NoError(t, err)
		assert.Equal(t, 13, n)
		assert.Equal(t, []byte("Hello, World!"), file.Bytes())
		assert.Equal(t, int64(13), file.pos)
	})

	t.Run("test write at non-zero position", func(t *testing.T) {
		file := NewFile([]byte("Hello, World!"), "test.txt")
		...
		assert.NoError(t, err)
		assert.Equal(t, 4, n)
		// Write at position 5 overwrites 4 characters starting from position 5
		// "Hello, World!" -> "HelloTestrld!" (positions 5-8 replaced with "Test")
		assert.Equal(t, []byte("HelloTestrld!"), file.Bytes())
		assert.Equal(t, int64(9), file.pos)
	})
}
