...
func TestNoPadding(t *testing.T) {
	t.Run("No padding success", func(t *testing.T) {
		data := []byte("Hello, World!")
		padded := NewNoPadding(data)
		assert.Equal(t, data, padded)
	})

	t.Run("No unpadding success", func(t *testing.T) {
		data := []byte("Hello, World!")
...
func TestEmptyPadding(t *testing.T) {
	t.Run("Empty padding with partial block", func(t *testing.T) {
		data := []byte("Hello")
		blockSize := 8
		expected := []byte("Hello\x00\x00\x00")
		padded := NewEmptyPadding(data, blockSize)
		assert.Equal(t, expected, padded)
	})

	t.Run("Empty padding with exact block size", func(t *testing.T) {
		data := []byte("12345678") // 8 bytes
		blockSize := 8
		padded := NewEmptyPadding(data, blockSize)
		assert.Equal(t, data, padded) // No padding added
	})

	t.Run("Empty padding with multiple blocks", func(t *testing.T) {
		data := []byte("1234567890123456") // 16 bytes
		blockSize := 8
		padded := NewEmptyPadding(data, blockSize)
		assert.Equal(t, data, padded) // No padding needed
	})

	t.Run("Empty padding with empty data", func(t *testing.T) {
		var data []byte
		blockSize := 8
	...
	t.Run("Empty unpadding with trailing zeros", func(t *testing.T) {
		data := []byte("Hello\x00\x00\x00")
		expected := []byte("Hello")
		unpadded := NewEmptyUnPadding(data)
		assert.Equal(t, expected, unpadded)
	})

	t.Run("Empty unpadding with no trailing zeros", func(t *testing.T) {
		data := []byte("Hello")
		unpadded := NewEmptyUnPadding(data)
		assert.Equal(t, data, unpadded)
	})

	t.Run("Empty unpadding with all zeros", func(t *testing.T) {
		data := []byte{0, 0, 0, 0}
		unpadded := NewEmptyUnPadding(data)
		assert.Equal(t, []byte{}, unpadded)
	})

	t.Run("Empty unpadding with empty data", func(t *testing.T) {
		var data []byte
		unpadded := NewEmptyUnPadding(data)
...
func TestZeroPadding(t *testing.T) {
	...
	t.Run("Zero padding with empty data", func(t *testing.T) {
		...
		blockSize := 8
		padded := NewZeroPadding(data, blockSize)
		assert.Nil(t, padded) // Empty data returns nil with zero padding
	})
...
func TestPaddingModes(t *testing.T) {
	...
	t.Run("Empty padding mode", func(t *testing.T) {
		assert.Equal(t, PaddingMode("Empty"), Empty)
	})
...
func TestPaddingRoundTrip(t *testing.T) {
	testCases := []struct {
		...
		blockSize   int
		test        func(t *testing.T, mode PaddingMode, blockSize int)
	}{
		{"No padding round trip", No, 8, testNoRoundTrip},
		{"Empty padding round trip", Empty, 8, testEmptyRoundTrip},
		{"Zero padding round trip", Zero, 8, testZeroRoundTrip},
		{"PKCS7 padding round trip", PKCS7, 8, testPKCS7RoundTrip},
		{"PKCS5 padding round trip", PKCS5, 8, testPKCS5RoundTrip},
		{"AnsiX923 padding round trip", AnsiX923, 8, testAnsiX923RoundTrip},
		{"ISO97971 padding round trip", ISO97971, 8, testISO97971RoundTrip},
		{"ISO10126 padding round trip", ISO10126, 8, testISO10126RoundTrip},
		{"ISO78164 padding round trip", ISO78164, 8, testISO78164RoundTrip},
		{"Bit padding round trip", Bit, 8, testBitRoundTrip},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.test(t, tc.paddingMode, tc.blockSize)
		})
	}
}

func testNoRoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello, World!")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
}

func testEmptyRoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
...
func testZeroRoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
...
func testPKCS7RoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
...
func testPKCS5RoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
...
func testAnsiX923RoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
...
func testISO97971RoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
...
func testISO10126RoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
...
func testISO78164RoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
...
func testBitRoundTrip(t *testing.T, mode PaddingMode, blockSize int) {
	original := []byte("Hello")
	padded := padding(original, mode, blockSize)
	unpadded := unpadding(padded, mode)
	assert.Equal(t, original, unpadded)
...
func TestPaddingEdgeCases(t *testing.T) {
	t.Run("Unknown padding mode", func(t *testing.T) {
		data := []byte("Hello")
		blockSize := 8
		unknownMode := PaddingMode("Unknown")
		padded := padding(data, unknownMode, blockSize)
		unpadded := unpadding(padded, unknownMode)
		assert.Equal(t, data, unpadded) // Should use No padding for unknown mode
	})
	t.Run("Empty data with all padding modes", func(t *testing.T) {
		blockSize := 8
		modes := []PaddingMode{No, Empty, Zero, PKCS7, PKCS5, AnsiX923, ISO97971, ISO10126, ISO78164, Bit}

		for _, mode := range modes {
			t.Run(string(mode), func(t *testing.T) {
				var data []byte
				padded := padding(data, mode, blockSize)
				unpadded := unpadding(padded, mode)
				// For empty data, some padding modes return nil, others return empty slice
				// Both are functionally equivalent in Go
				if unpadded == nil {
					assert.Nil(t, unpadded)
				} else {
					assert.Equal(t, []byte{}, unpadded)
	...
	t.Run("Exact block size with all padding modes", func(t *testing.T) {
		blockSize := 8
		modes := []PaddingMode{No, Empty, Zero, PKCS7, PKCS5, AnsiX923, ISO97971, ISO10126, ISO78164, Bit}

		for _, mode := range modes {
			t.Run(string(mode), func(t *testing.T) {
				data := []byte("12345678") // Exactly 8 bytes
				padded := padding(data, mode, blockSize)
				unpadded := unpadding(padded, mode)
				assert.Equal(t, data, unpadded)
			})
	...
	t.Run("Large data with PKCS7 padding", func(t *testing.T) {
		// Create data that's multiple blocks
		data := make([]byte, 24) // 3 blocks of 8 bytes
		for i := range data {
			data[i] = byte(i)
		}

		blockSize := 8
		padded := padding(data, PKCS7, blockSize)
		unpadded := unpadding(padded, PKCS7)
		assert.Equal(t, data, unpadded)
	})
}
