...
func TestBlockCipher_Decrypt(t *testing.T) {
	...
	t.Run("decrypt with error handling", func(t *testing.T) {
		// Test unknown block mode case
		cipher := &blockCipher{
			Block:   BlockMode("UNKNOWN"), // Unknown block mode
			Padding: PKCS7,
			IV:      testIV,
		...
		// Decryption should return nil result because block mode is unknown
		result, err := cipher.Decrypt(testData, block)
		assert.NoError(t, err)
		assert.Nil(t, result)
	})
	...
	t.Run("decrypt with decryption error", func(t *testing.T) {
		...
		// Decryption with unknown block mode should return nil without error
		result, err := cipher.Decrypt(testData, block)
		assert.NoError(t, err)
		assert.Nil(t, result)
	})
