...
func TestAdditionalCoverage(t *testing.T) {
	...
	t.Run("test encrypter cipher creation error", func(t *testing.T) {
		var buf bytes.Buffer
		c := cipher.NewDesCipher(cipher.CBC)
		// Use an invalid key that will pass length check but fail cipher creation
		// This is tricky for DES since des.NewCipher rarely fails for 8-byte keys
		c.SetKey(key8)
		c.SetIV(iv8)
		c.SetPadding(cipher.PKCS7)
	...
	t.Run("test decrypter read with empty encrypted data", func(t *testing.T) {
	...
	t.Run("test NewStreamEncrypter with cipher creation error", func(t *testing.T) {
		var buf bytes.Buffer
		c := cipher.NewDesCipher(cipher.CBC)
		// Use a key that will pass length check but fail cipher creation
		// This is very difficult for DES, so we'll manually corrupt the cipher
		c.SetKey(key8)
		c.SetIV(iv8)
		c.SetPadding(cipher.PKCS7)
	...
	t.Run("test NewStreamEncrypter with invalid key length", func(t *testing.T) {
		var buf bytes.Buffer
		c := cipher.NewDesCipher(cipher.CBC)
		// Use an invalid key length to trigger KeySizeError
		c.SetKey([]byte("short"))
		c.SetIV(iv8)
		c.SetPadding(cipher.PKCS7)
