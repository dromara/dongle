...
// TestFormatKeyBody tests the formatKeyBody function
func TestFormatKeyBody(t *testing.T) {
	t.Run("format short key body", func(t *testing.T) {
		keyBody := []byte("short")
		header := "-----BEGIN TEST-----\n"
		tail := "-----END TEST-----\n"

		result := formatKeyBody(keyBody, header, tail)
		expected := "-----BEGIN TEST-----\nshort\n-----END TEST-----\n"
		assert.Equal(t, expected, string(result))
	})

	t.Run("format key body exactly 64 characters", func(t *testing.T) {
		keyBody := []byte("1234567890123456789012345678901234567890123456789012345678901234")
		header := "-----BEGIN TEST-----\n"
		tail := "-----END TEST-----\n"
	...
	t.Run("format key body longer than 64 characters", func(t *testing.T) {
		keyBody := []byte("123456789012345678901234567890123456789012345678901234567890123456789")
		header := "-----BEGIN TEST-----\n"
		tail := "-----END TEST-----\n"
	...
	t.Run("format empty key body", func(t *testing.T) {
		keyBody := []byte{}
		header := "-----BEGIN TEST-----\n"
		tail := "-----END TEST-----\n"
	...
	t.Run("format key body with special characters", func(t *testing.T) {
		keyBody := []byte("test+key/with=special&chars")
		header := "-----BEGIN TEST-----\n"
		tail := "-----END TEST-----\n"
