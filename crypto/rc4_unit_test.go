...
func TestRc4InputTypes(t *testing.T) {
	t.Run("string input", func(t *testing.T) {
		...
		encrypted := NewEncrypter().FromString(plaintext).ByRc4(key).ToRawString()
		assert.NotEmpty(t, encrypted)

		decrypted := NewDecrypter().FromRawString(encrypted).ByRc4(key).ToString()
	...
	t.Run("bytes input", func(t *testing.T) {
		...
		encrypted := NewEncrypter().FromBytes(plaintext).ByRc4(key).ToRawBytes()
		assert.NotEmpty(t, encrypted)

		decrypted := NewDecrypter().FromRawBytes(encrypted).ByRc4(key).ToBytes()
		assert.Equal(t, plaintext, decrypted)
	})
...
func TestRc4ErrorHandling(t *testing.T) {
	t.Run("empty key", func(t *testing.T) {
		...
		encrypted := NewEncrypter().FromString(plaintext).ByRc4([]byte{}).ToRawString()
		assert.Empty(t, encrypted)

		decrypted := NewDecrypter().FromRawString(plaintext).ByRc4([]byte{}).ToString()
	...
	t.Run("key too large", func(t *testing.T) {
		...
		decrypted := NewDecrypter().FromRawString(plaintext).ByRc4(key).ToString()
		assert.Empty(t, decrypted)
	})
	...
	t.Run("with existing error", func(t *testing.T) {
		...
		encrypted := encrypter.FromString(plaintext).ByRc4(key).ToRawString()
		assert.Empty(t, encrypted)

		decrypter := NewDecrypter()
		decrypter.Error = assert.AnError

		decrypted := decrypter.FromRawString(plaintext).ByRc4(key).ToString()
		assert.Empty(t, decrypted)
	})
...
func TestRc4Streaming(t *testing.T) {
	t.Run("stream encrypter with valid key", func(t *testing.T) {
		...
		encrypted := NewEncrypter().FromFile(mockFile).ByRc4(key).ToRawString()
		assert.NotEmpty(t, encrypted)

		// For decryption, we need to use the encrypted data directly
		// since Decrypter doesn't have FromFile method
		decrypted := NewDecrypter().FromRawBytes([]byte(encrypted)).ByRc4(key).ToString()
		assert.Equal(t, plaintext, decrypted)
	})
	...
	t.Run("stream with read error", func(t *testing.T) {
		...
		encrypted := NewEncrypter().FromFile(errorReader).ByRc4(key).ToRawString()
		assert.Empty(t, encrypted)
	})
...
func TestRc4StdDecrypter(t *testing.T) {
	...
	t.Run("std decrypter decrypt empty bytes", func(t *testing.T) {
		...
		decrypted := NewDecrypter().FromRawBytes(plaintext).ByRc4(key).ToBytes()
		assert.Equal(t, plaintext, decrypted)
	})
...
func TestRc4ByRc4StreamBranch(t *testing.T) {
	...
	t.Run("decrypter stream branch", func(t *testing.T) {
		...
		decrypted := decrypter.ByRc4(key).ToString()
		assert.Equal(t, plaintext, decrypted)
	})
}
