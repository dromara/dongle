...
head:
  - - meta
    - name: description
      content: AES 加密算法|一个轻量级、语义化、对开发者友好的 golang 编码&密码库
  - - meta
    - name: keywords
      content: aes, 加密, 解密, 对称加密, 分组密码
---

# AES

AES（Advanced Encryption Standard）是一种对称加密算法，支持 `16` 字节、`24` 字节和 `32` 字节密钥长度。`dongle` 支持标准 `AES` 加密，提供多种分组模式、填充模式和输出格式。
