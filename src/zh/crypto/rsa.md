...
- **PKCS1**：PKCS#1 格式，密钥使用 `-----BEGIN RSA PRIVATE KEY-----` 和 `-----BEGIN RSA PUBLIC KEY-----` 作为头尾 
- **PKCS8**：PKCS#8 格式，密钥使用 `-----BEGIN PRIVATE KEY-----` 和 `-----BEGIN PUBLIC KEY-----` 作为头尾 （推荐）

支持以下填充模式：

- **PKCS1v15**：PKCS#1 v1.5 填充模式，可用于加密和签名，兼容性好
- **OAEP**：最优非对称加密填充，仅用于加密/解密，安全性更高（推荐用于加密）
...
- **密钥格式**：推荐使用 `PKCS8` 格式（现代标准）
- **填充模式**：推荐使用 `OAEP` 进行加密/解密
...
kp.SetPadding(keypair.OAEP)  // 或 keypair.PKCS1v15
