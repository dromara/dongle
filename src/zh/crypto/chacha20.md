...
head:
  - - meta
    - name: description
      content: ChaCha20 流密码加密算法，支持 32 字节密钥与 12 字节随机数（Nonce），无需填充即可处理任意长度数据，支持标准和流式处理，支持 Hex 和 Base64 输出格式
  - - meta
    - name: keywords
      content: dongle, go-dongle, 加密, 解密, ChaCha20, 对称加密算法, 流密码, Nonce, 流式处理, Hex, Base64
---
