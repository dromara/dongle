...
head:
  - - meta
    - name: description
      content: AES (Advanced Encryption Standard) 対称暗号化アルゴリズム、16、24 または 32 バイトキーをサポート、多様なブロックモード（CBC、ECB、CTR、GCM、CFB、OFB）とパディングモードを提供、標準処理とストリーム処理をサポート、Hex および Base64 出力形式をサポート
  - - meta
    - name: keywords
      content: dongle, go-dongle, 暗号化, 復号化, AES, 対称暗号化アルゴリズム, ブロックモード, パディングモード, CBC, ECB, CTR, GCM, CFB, OFB
---
...
- **GCM（Galois/Counter Mode）**：ガロア/カウンターモード、鍵 `Key`、乱数 `Nonce`（1-255バイト）、追加認証データ `AAD`（オプション）の設定が必要
...
> **注意**：CFBモードはCFB8実装を使用します。最初の16バイトのデータについて、CFB8とOFBモードは同じ暗号化結果を生成します。これはGo標準ライブラリのCFB8実装の特性であり、エラーではありません。
