...
head:
  - - meta
    - name: description
      content: RSA 非対称暗号化アルゴリズム。PKCS1 と PKCS8 鍵形式をサポートし、複数のハッシュアルゴリズムをサポート。公開鍵で暗号化し、秘密鍵で復号化。標準処理とストリーム処理をサポートし、Hex および Base64 出力形式をサポートします
  - - meta
    - name: keywords
      content: dongle, go-dongle, 暗号化, 復号化, RSA, 非対称暗号化アルゴリズム, 公開鍵暗号化, 秘密鍵復号化, PKCS1, PKCS8, PSS, PKCS1v15
---
...
- **PKCS1**：PKCS#1形式、鍵は `-----BEGIN RSA PRIVATE KEY-----` と `-----BEGIN RSA PUBLIC KEY-----` をヘッダーとフッターとして使用
- **PKCS8**：PKCS#8形式、鍵は `-----BEGIN PRIVATE KEY-----` と `-----BEGIN PUBLIC KEY-----` をヘッダーとフッターとして使用（推奨）

以下のパディングモードをサポート：

- **PKCS1v15**：PKCS#1 v1.5 パディングモード、暗号化と署名に使用可能、互換性が良い
- **OAEP**：最適非対称暗号化パディング、暗号化/復号化のみに使用、より高いセキュリティ（暗号化に推奨）
...
- **鍵形式**：`PKCS8` 形式の使用を推奨（現代標準）
- **パディングモード**：暗号化/復号化には `OAEP` の使用を推奨
...
// パディングモードを設定（オプション、デフォルトは空、PKCS1形式はデフォルトでPKCS1v15、PKCS8形式はデフォルトでOAEP）
kp.SetPadding(keypair.OAEP)  // または keypair.PKCS1v15
// ハッシュアルゴリズムを設定（オプション、デフォルトはSHA256、OAEPパディングモード用）
