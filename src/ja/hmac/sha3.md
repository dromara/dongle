...
head:
  - - meta
    - name: description
      content: HMAC-SHA3 メッセージ認証コードアルゴリズム、SHA3 ハッシュアルゴリズムに基づき、キーを使用してメッセージ認証を行う、sha3-224、sha3-256、sha3-384 と sha3-512 の4つのバリアントを提供、標準およびストリーミング処理をサポート、文字列、バイト、ファイル入力をサポート、Hex および Base64 出力形式をサポート
  - - meta
    - name: keywords
      content: dongle, go-dongle, HMAC, メッセージ認証コード, SHA3, sha3-224, sha3-256, sha3-384, sha3-512, キー, ストリーミング処理, 文字列入力, バイト入力, ファイル入力, Hex, Base64
---
