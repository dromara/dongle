...
func TestHasher_ByMd4(t *testing.T) {
	t.Run("hash string", func(t *testing.T) {
		hasher := NewHasher().FromString("hello").ByMd4()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, []byte{0x86, 0x64, 0x37, 0xcb, 0x7a, 0x79, 0x4b, 0xce, 0x2b, 0x72, 0x7a, 0xcc, 0x03, 0x62, 0xee, 0x27}, hasher.dst)
	})

	t.Run("hash bytes", func(t *testing.T) {
		hasher := NewHasher().FromBytes([]byte("hello")).ByMd4()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, []byte{0x86, 0x64, 0x37, 0xcb, 0x7a, 0x79, 0x4b, 0xce, 0x2b, 0x72, 0x7a, 0xcc, 0x03, 0x62, 0xee, 0x27}, hasher.dst)
	})
	...
	t.Run("unicode data", func(t *testing.T) {
		hasher := NewHasher().FromString("你好世界").ByMd4()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})
...
func TestHasher_ByMd4_HMAC(t *testing.T) {
	t.Run("hmac with key", func(t *testing.T) {
		hasher := NewHasher().FromString("hello").WithKey([]byte("secret")).ByMd4()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})

	t.Run("hmac with bytes", func(t *testing.T) {
		hasher := NewHasher().FromBytes([]byte("hello")).WithKey([]byte("secret")).ByMd4()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})
	...
	t.Run("hmac with file", func(t *testing.T) {
		file := mock.NewFile([]byte("hello"), "test.txt")
		hasher := NewHasher().FromFile(file).WithKey([]byte("secret")).ByMd4()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})
	...
	t.Run("hmac with large key", func(t *testing.T) {
		...
		hasher := NewHasher().FromString("hello").WithKey(key).ByMd4()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})
}
...
func TestHasher_ByMd4_Error(t *testing.T) {
	...
	t.Run("error with key", func(t *testing.T) {
		...
		result := hasher.WithKey([]byte("secret")).ByMd4()
	...
	t.Run("hmac with empty key", func(t *testing.T) {
		hasher := NewHasher().FromString("hello").WithKey([]byte{}).ByMd4()
		assert.NotNil(t, hasher.Error)
		assert.Contains(t, hasher.Error.Error(), "hmac: key cannot be empty")
	})

	t.Run("hmac with nil key", func(t *testing.T) {
		hasher := NewHasher().FromString("hello").WithKey(nil).ByMd4()
		assert.NotNil(t, hasher.Error)
		assert.Contains(t, hasher.Error.Error(), "hmac: key cannot be empty")
	})
}
