...
func TestHasher_ByMd2(t *testing.T) {
	t.Run("hash string", func(t *testing.T) {
		hasher := NewHasher().FromString("hello").ByMd2()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, []byte{0xa9, 0x04, 0x6c, 0x73, 0xe0, 0x03, 0x31, 0xaf, 0x68, 0x91, 0x7d, 0x38, 0x04, 0xf7, 0x06, 0x55}, hasher.dst)
	})

	t.Run("hash bytes", func(t *testing.T) {
		hasher := NewHasher().FromBytes([]byte("hello")).ByMd2()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, []byte{0xa9, 0x04, 0x6c, 0x73, 0xe0, 0x03, 0x31, 0xaf, 0x68, 0x91, 0x7d, 0x38, 0x04, 0xf7, 0x06, 0x55}, hasher.dst)
	})
	...
	t.Run("unicode data", func(t *testing.T) {
		hasher := NewHasher().FromString("你好世界").ByMd2()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})
...
func TestHasher_ByMd2_HMAC(t *testing.T) {
	t.Run("hmac with key", func(t *testing.T) {
		hasher := NewHasher().FromString("hello").WithKey([]byte("secret")).ByMd2()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})

	t.Run("hmac with bytes", func(t *testing.T) {
		hasher := NewHasher().FromBytes([]byte("hello")).WithKey([]byte("secret")).ByMd2()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})
	...
	t.Run("hmac with file", func(t *testing.T) {
		file := mock.NewFile([]byte("hello"), "test.txt")
		hasher := NewHasher().FromFile(file).WithKey([]byte("secret")).ByMd2()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})
	...
	t.Run("hmac with large key", func(t *testing.T) {
		...
		hasher := NewHasher().FromString("hello").WithKey(key).ByMd2()
		assert.Nil(t, hasher.Error)
		assert.Equal(t, 16, len(hasher.dst))
	})
}
...
func TestHasher_ByMd2_Error(t *testing.T) {
	...
	t.Run("error with key", func(t *testing.T) {
		...
		result := hasher.WithKey([]byte("secret")).ByMd2()
	...
	t.Run("hmac with empty key", func(t *testing.T) {
		hasher := NewHasher().FromString("hello").WithKey([]byte{}).ByMd2()
		assert.NotNil(t, hasher.Error)
		assert.Contains(t, hasher.Error.Error(), "hmac: key cannot be empty")
	})

	t.Run("hmac with nil key", func(t *testing.T) {
		hasher := NewHasher().FromString("hello").WithKey(nil).ByMd2()
		assert.NotNil(t, hasher.Error)
		assert.Contains(t, hasher.Error.Error(), "hmac: key cannot be empty")
	})
}
